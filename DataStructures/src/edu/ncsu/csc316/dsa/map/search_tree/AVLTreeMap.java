...
public class AVLTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
	...
	private void rebalance(Position<Entry<K, V>> p) {
		int oldHeight = 0;
		int newHeight;
		Position<Entry<K, V>> original = p;
		do {
			oldHeight = getProperty(original);
			if (!isBalanced(original)) {
				
				// Find the grandchild with the "taller" height
				Position<Entry<K, V>> child = tallerChild(original);
				// Find the grandchild with the "taller" height
				...
				original = restructure(grandchild);
				recomputeHeight(left(original));
				recomputeHeight(right(original));
			}
			recomputeHeight(original);
			newHeight = getProperty(original);
			// MOve up to the parent
			original = parent(original);
		} while (oldHeight != newHeight && original != null);
	}
